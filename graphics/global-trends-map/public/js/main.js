...
/***/ function(module, exports, __webpack_require__) {
	...
	var GRAPHICINFO = __webpack_require__(2);
	...
	*/
	...
	var colorScale = function colorScale(input) {
	  var r;
	...
	function draw(err, data) {
	  ...
	  var projection = d3.geo.naturalEarth().scale(width / 6).center(center).translate([width / 1.8, height / 2.8]);
	
	  path = d3.geo.path().projection(projection);
	
	  var graticule = d3.geo.graticule();
	
	  var svg = d3.select($graphic[0]).append("svg").attr("width", width).attr("height", height).attr("fill", "rgba(255, 255, 255, 0.5)").attr("style", "background: black");
	  ...
	  svg.append('path').datum(topojson.merge(data, data.objects["ne_110m_land"].geometries)).attr("class", "country-shape").attr('fill', 'rgba(255, 255, 255, 0.25)').attr("d", path);
	  addLegend();
	...
	function removeOgallalaOutline() {
	  ...
	  var label = map.select('.gig-aquifer-label').transition().duration(transisitonDuration).attr('opacity', 0);
	
	  var shape = map.select('.ogallala-shape').transition().duration(transisitonDuration).attr('opacity', 0);
	...
	function addOgallalaOutline() {
	  ...
	  var textLabel = map.append('g').attr('class', 'gig-aquifer-label').attr('transform', 'translate(' + projection([-95.295983, 39.464040]) + ')').append('text').attr('transform', 'translate(-10, 0)').attr('fill', 'white').attr('font-size', 14).text("Ogallala Aquifer");
	
	  var shape = map.append("path").data(ogallala_data).attr("class", "ogallala-shape").attr("stroke", "white").attr("stroke-width", 1).attr("fill", "rgba(255, 255, 255, 0.9)").attr("opacity", 0).attr("d", path);
	  shape.transition().duration(transisitonDuration).attr('opacity', 0.8);
	}
	...
	function showHaskell() {
	  ...
	  map.append('g').attr('class', 'county-detail-text').attr('transform', 'translate(' + projection([-100.851404, 37.482529]) + ')').append('text').attr("fill", "white").attr('transform', 'translate(10, 5)').text('Haskell County, KS').attr('font-size', 10);
	...
	function zoomIn(center, zoomLevel) {
	  ...
	  map.transition().duration(transisitonDuration).attr("transform", "translate(" + (width / 2 - coordinates[0] * zoomLevel) + ", " + (HEIGHT / 2 - coordinates[1] * zoomLevel) + ")scale(" + zoomLevel + ")");
	}
	
	function zoomOut() {
	  map.transition().duration(transisitonDuration).attr("transform", "");
	}
	...
	function mousemove() {
	  ...
	  // }
	  tooltip.style("left", d3.event.pageX + "px").style("top", top + "px");
	}
...
/***/ function(module, exports) {
	...
	module.exports = {
		"HEADLINE": "Global",
		"GRAPHIC_SLUG": "global-map",
		"CHATTER": "Map of changing water levels in India",
		"DATA_URL": "http://www.gannett-cdn.com/experiments/usatoday/2015/groundwater/graphics/india-map/data/world_aquifers.topo.json",
		"FULL_WIDTH": true,
		"SOURCE": "USGS",
		"SLIDER_IMAGES": [
			{
				"title": "Global ground water facts",
				"img": "http://www.gannett-cdn.com/experiments/usatoday/2015/groundwater/graphics/gw-slider/img/heheh.gif"
			},
			{
				"title": "More groundwater facts",
				"img": "http://www.gannett-cdn.com/experiments/usatoday/2015/groundwater/graphics/gw-slider/img/unsplash-2.jpg"
			},
			{
				"title": "Aquifers",
				"img": "http://www.gannett-cdn.com/experiments/usatoday/2015/groundwater/graphics/gw-slider/img/unsplash-2.jpg"
