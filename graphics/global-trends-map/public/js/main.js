...
/***/ function(module, exports, __webpack_require__) {
	...
	function start() {
	  ...
	  queue().defer(d3.json, DATA_URL).defer(d3.json, "data/split.json").await(ready);
	...
	function ready(err, data, data2) {
	  ...
	  var svg = d3.select("#" + GRAPHICINFO.GRAPHIC_SLUG).append("svg").attr("width", width).attr("height", height).attr("fill", "rgba(255, 255, 255, 0.5)").attr("style", "background: black");
	  ...
	  svg.append("g").selectAll("path").data(data2.features).enter().append("path").attr("fill", function (d) {
	...
	function mousemove() {
	  tooltip.style("left", d3.event.pageX + "px").style("top", d3.event.pageY - 12 + "px");
	}
