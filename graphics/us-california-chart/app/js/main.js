...
var GRAPHICDATA2;
var chartCurtain;
var hasAnimated = false; //boolean to store whether or not chart has animated. we will only animte once.
...
function updatePosition(e) {
  ...
  if (!hasAnimated && progressBottom > 0) {
    animateChart();
  }
  ...
  // } else {
    // previousStep();
  // }
   if (progressBottom >= 1 && slidesLength > 1) {
...
function ready(err, data, redraw) {
    width = $(window).width();
    height = width * (9/16);
    scale = width * 1.5;
    if (width < 800) {
...
function drawLineChart(linedata, redraw) {
  /*
...
chart = d3.select($graphic[0])
  ...
  chart.append("path")
      .datum(linedata)
      .attr('fill', 'none')
      .attr('stroke', scaleColors[2])
      .attr("class", "line")
      .attr("d", line);

  chartCurtain = chart.append('rect')
    .attr('x', -1 * width)
    .attr('y', -1 * height)
    .attr('height', height)
    .attr('width', width)
  ...
  chart.append('text')
    .attr('fill', 'black')
    .attr('y', height + 40)
    .text('Source: ' + GRAPHICINFO.SOURCE)
}
